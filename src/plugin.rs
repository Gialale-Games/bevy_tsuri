...
fn tick_interrupt_components(
    ...
    // Despawn any leftover temp entities from previous frames that never got a response
    for (temp_entity, _) in interrupt_state.pending_interrupts.drain() {
        commands.entity(temp_entity).try_despawn();
    }
