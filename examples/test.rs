...
fn get_random_enemy_vals() -> (f32, f32, f32) {
    let x = rand::random::<f32>() * 1000.0 - 5000.0;
    let y = rand::random::<f32>() * 1000.0 - 5000.0;
    let vision_radius =
...
fn handle_shortcuts(mut commands: Commands, keys: Res<ButtonInput<KeyCode>>) {
    if keys.pressed(KeyCode::KeyS) {
        commands.trigger(SpawnEnemies(1));
    }
    if keys.pressed(KeyCode::KeyD) {
        commands.trigger(DespawnEnemies(1));
    }
}
